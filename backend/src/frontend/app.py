...
if "transits" not in st.session_state:
    st.session_state["transits"] = None

# --- SIDEBAR MODE SELECTOR ---
mode = st.sidebar.radio(
    "Select Mode", ["👤 Individual Destiny", "❤️ Relationship Match"]
)
...
if mode == "👤 Individual Destiny":
    with st.sidebar:
        ...
        lat = st.number_input("Lat", 28.61)
        lon = st.number_input("Lon", 77.20)
        tz = st.number_input("TZ", 5.5)
        ...
        if st.button("Check Transits"):
            ...
            if res.status_code == 200:
                st.session_state["transits"] = res.json()["transits"]
    ...
    if "data" in st.session_state:
        ...
        if st.session_state["transits"]:
            with st.expander("🌌 Today's Sky (Transits)", expanded=True):
                ...
                for i, tr in enumerate(st.session_state["transits"]):
                    icon = (
                        "🪐" if tr["planet"] in ["Saturn", "Jupiter", "Mars"] else "⚪"
                    )
                    cols[i % 3].info(
                        f"**{icon} {tr['planet']}** in H{tr['transiting_house']}\n\n{tr['prediction']}"
                    )
        ...
        t1, t2, t3, t4, t5 = st.tabs(
            ["🔮 Analysis", "⏳ Timeline", "📐 Charts", "💬 Chat", "🧘 Yogas"]
        )

        # TAB 1: AI Reading
        with t1:
            st.markdown(data["ai_reading"])
        ...
        with t2:
            ...
            md_opts = [
                f"{m['lord']} ({m['start']} ➝ {m['end']})" for m in full_timeline
            ]
            sel_md_idx = st.selectbox(
                "Select Mahadasha",
                range(len(md_opts)),
                format_func=lambda x: md_opts[x],
            )
            ...
            if "sub_periods" in curr_md and curr_md["sub_periods"]:
                ...
                sel_ad_idx = st.selectbox(
                    f"Select Antardasha",
                    range(len(ad_opts)),
                    format_func=lambda x: ad_opts[x],
                )
                curr_ad = ad_list[sel_ad_idx]

                st.dataframe(
                    pd.DataFrame(ad_list).drop(
                        columns=["sub_periods", "type"], errors="ignore"
                    ),
                ...
                if "sub_periods" in curr_ad and curr_ad["sub_periods"]:
                    ...
                    pd_opts = [
                        f"{p['lord']} ({p['start']} ➝ {p['end']})" for p in pd_list
                    ]
                    sel_pd_idx = st.selectbox(
                        f"Select Pratyantar",
                        range(len(pd_opts)),
                        format_func=lambda x: pd_opts[x],
                    )
                    curr_pd = pd_list[sel_pd_idx]
                    st.dataframe(
                        pd.DataFrame(pd_list).drop(
                            columns=["sub_periods", "type"], errors="ignore"
                        ),
                        use_container_width=True,
                    )
        ...
        with t3:
            ...
            st.caption(
                "Visual representation of the Rashi (D1) and Navamsa (D9) charts."
            )
            ...
            if "planets" in data and "Ascendant" in data["planets"]:
                ...
                # D1 Chart
                d1_buf = draw_north_indian_chart(
                    data["planets"], asc_id, "D1 Rashi (Birth Chart)"
                )
                ...
                d9_buf = draw_north_indian_chart(
                    d9_planets, d9_asc_id, "D9 Navamsa (Strength)"
                )

                c1, c2 = st.columns(2)
                with c1:
                    st.image(d1_buf, use_container_width=True)
                with c2:
                    st.image(d9_buf, use_container_width=True)
        ...
        with t4:
            ...
            if q := st.chat_input("Ask PanditAI..."):
                ...
                res = requests.post(
                    f"{BASE}/chat", json={"query": q, "context": ctx}
                ).json()["response"]
        ...
        with t5:
            ...
            st.caption(
                "Special auspicious and inauspicious patterns found in your chart."
            )
            ...
            if "yogas" in data and data["yogas"]:
                ...
                for cat, yoga_list in categories.items():
                    with st.expander(
                        f"📌 {cat} Yogas ({len(yoga_list)})", expanded=True
                    ):
                        for y in yoga_list:
...
# ==========================================
    ...
    with c1:
        ...
        d1 = st.date_input("Date A", datetime(1990, 1, 1))
        t1 = st.time_input("Time A", time(12, 0))
    with c2:
        st.subheader("Partner B")
        d2 = st.date_input("Date B", datetime(1995, 1, 1))
        t2 = st.time_input("Time B", time(12, 0))
