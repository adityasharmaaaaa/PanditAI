...
class PDFReportGenerator:
    ...
    def create_report(self, user_name, data):
        ...
        elements.append(
            Paragraph(f"Vedic Destiny Report for {user_name}", self.styles["Heading2"])
        )
        ...
        # --- 2. PLANETARY TABLE ---
        elements.append(
            Paragraph("🪐 Planetary Coordinates", self.styles["SectionHeader"])
        )
        ...
        elements.append(
            Paragraph("📜 Detailed Life Analysis", self.styles["PanditTitle"])
        )

        if data.get("ai_reading"):
            reading = data["ai_reading"]

            # CASE A: Structured Dict (New)
            if isinstance(reading, dict):
                for category, text in reading.items():
                    # Capitalize Title
                    title = category.replace("_", " ").title()
                    elements.append(Paragraph(title, self.styles["SectionHeader"]))

                    # Add Text body
                    elements.append(Paragraph(str(text), self.styles["BodyTextCustom"]))
                    elements.append(Spacer(1, 10))

            # CASE B: Raw String (Legacy/Fallback)
        ...
        # --- 4. DASHA TIMELINE ---
        elements.append(
            Paragraph("⏳ Vimshottari Dasha Timeline", self.styles["SectionHeader"])
        )
